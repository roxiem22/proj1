...
)
{
    ...
    if (pThread->TickCountCompleted > 16) {
        if (++pCpu->ThreadData.RunningThreadTicks >= THREAD_TIME_SLICE_2)
        {
            pThread->noOfQuanta++;
            LOG_TRACE_THREAD(" Will yield on return \ n ");
...
)
{
    ...
    LockAcquire(&m_threadSystemData.ReadyThreadsLock, &oldState);
    //added
    LOG(" Thread 0 x % X was allocated % llu quanta of time until completion ",
        pThread->Id, pThread->noOfQuanta);
    _ThreadSchedule();
    NOT_REACHED;
}
